...
public class validSudoku {
    ...
    public static boolean isValidSudoku(char[][] board) {
        ...
        for (int i = 0; i < len; i++) {
            for (int j = 0; j < len; j++) {
                if (board[i][j] != '.') {
                    if (!set.add(i + "row" + board[i][j]) || !set.add(j + "col" + board[i][j])
                            || !set.add(i / 3 + " " + j / 3 + "box" + board[i][j]))
                        return false;
                    // if (set.contains(i + "row" + board[i][j]) || set.contains(j + "col" +
                    // board[i][j]) || set.contains(i/3 + " " + j/3 + "box" + board[i][j])) return
                    // false;
